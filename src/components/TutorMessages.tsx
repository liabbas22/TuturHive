...
const TutorMessages: React.FC = () => {
  ...
  const formatTime = (dateString?: string | null) => {
    ...
    if (diffInHours < 168) return date.toLocaleDateString([], { weekday: "short", hour: "2-digit", minute: "2-digit" });
  ...
  useEffect(() => {
    ...
    const onNewMessage = (message: Message) => {
      ...
      if (message.sender?._id !== currentUserId && Notification.permission === "granted") {
        try {
          ...
          new Notification("New Message", { body: `${senderName}: ${message.content}`, icon: "/favicon.ico" });
        } catch {}
      }
    };

    const onMessageSent = (data: { _id: string; status: string; createdAt: string; realId?: string }) => {
      setMessages((prev) =>
        prev.map((msg) =>
          msg._id === data._id || msg._id === data.realId
            ? { ...msg, status: data.status as any, createdAt: data.createdAt || msg.createdAt, _id: data._id }
            : msg
        )
      );
    };
    ...
    const onMessagesRead = (data: { byUserId: string; readAt: string }) => {
      setMessages((prev) =>
        prev.map((msg) => (msg.sender?._id === currentUserId && msg.receiver?._id === data.byUserId ? { ...msg, status: "read", isRead: true } : msg))
      );
    };
  ...
  useEffect(() => {
    ...
    socket?.emit("joinPrivateRoom", { user1: currentUserId, user2: selectedChat });
  ...
  const markAsRead = async (otherUserId: string) => {
    try {
      ...
      setMessages((prev) => prev.map((m) => (m.sender._id === otherUserId ? { ...m, isRead: true, status: "read" } : m)));
      loadUnreadCount();
    } catch {}
  };
  ...
  const handleSendMessage = async () => {
    ...
    if (socket && socket.connected) {
      ...
      try {
        ...
        if (saved) setMessages((prev) => prev.map((m) => (m._id === tempMessage._id ? saved : m)));
      } catch {}
    }
  };
  ...
  const getMessageStatusIcon = (status: string, isRead: boolean) => {
    if (isRead) return <CheckCheck className="h-4 w-4 text-[blue]" />;
  ...
  return (
    <div className="bg-white rounded-lg shadow-lg h-full max-h-screen md:max-h-[600px] flex flex-col overflow-hidden">
      <div className="flex flex-1 flex-col md:flex-row overflow-hidden">
            ...
            {conversations.map((conversation) => (
              <div
                ...
                className={`p-4 border-b border-gray-100 cursor-pointer hover:bg-gray-100 transition-colors ${
                  selectedChat === conversation._id ? "bg-green-50 border-green-200" : ""
                }`}
              >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {conversation.unreadCount > 0 && (
                      <div className="flex justify-end mt-1">
                        <span className="bg-green-500 text-white text-xs rounded-full px-2 py-1 min-w-[20px] text-center">{conversation.unreadCount}</span>
                      </div>
                    )}
                  </div>
                </div>
        ...
        <div className="flex-1 flex flex-col bg-gray-100 overflow-hidden w-full">
          {selectedChat ? (
            <>
              <div className="p-2 border-b border-gray-200 bg-white shadow-sm flex items-center space-x-3 overflow-x-hidden">
                  ...
                  messages.map((message) => {
                    ...
                    return (
                      <div key={message._id} className={`flex ${isOwnMessage ? "justify-end" : "justify-start"}`}>
                        <div className={`max-w-xs sm:max-w-sm lg:max-w-md px-4 py-2 rounded-2xl ${isOwnMessage ? "bg-blue-500 text-white rounded-br-md" : "bg-white text-gray-900 rounded-bl-md shadow-sm"}`}>
              ...
              <div className="p-4 bg-white border-t border-gray-200">
                <div className="flex items-center space-x-2">
                  <input
                    ...
                    className="flex-1 px-4 py-3 border border-gray-300 rounded-full focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent placeholder-gray-400"
                  />
                  <button
